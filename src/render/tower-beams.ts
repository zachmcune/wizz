...
function drawFlameBeam(
  ...
  for (let layer = 0; layer < 4; layer++) {
    ...
    for (let i = 1; i <= segments; i++) {
      const t = i / segments;
      const flicker = Math.sin(phase * 7 + i * 1.3 + seed) * (4 + layer * 2);
    ...
    for (let i = segments; i >= 0; i--) {
      const t = i / segments;
      const flicker = Math.sin(phase * 7 + i * 1.3 + seed + 2) * (4 + layer * 2);
      const px = sx + dx * t + nx * flicker * (0.15 + layer * 0.08);
      const py = sy + dy * t + ny * flicker * (0.15 + layer * 0.08);
      const half = width * (0.35 + t * 0.65) * 0.5;
  ...
  for (let i = 1; i <= segments; i++) {
    const t = i / segments;
    const flicker = Math.sin(phase * 9 + i * 2.1 + seed) * 2.5;
    core.lineTo(sx + dx * t + nx * flicker * 0.06, sy + dy * t + ny * flicker * 0.06);
  }
  core.stroke({ width: Math.max(2, startW * 0.18), color: 0xffffff, alpha: 0.92 });

  // Embers along the stream
  for (let i = 0; i < 8; i++) {
    const t = detRand(seed + i * 3.7) * 0.85 + 0.05;
    const drift = Math.sin(phase * 3 + i * 1.7) * 6;
    const px = sx + dx * t + nx * drift;
    const py = sy + dy * t + ny * drift;
    const r = 1.2 + detRand(seed + i) * 2.2;
  ...
  for (let i = 0; i < 5; i++) {
    ...
    const smy = ey + Math.sin(ang) * dist - Math.abs(Math.sin(phase + i)) * 8;
    fillPool.acquire().circle(smx, smy, 3 + detRand(seed + i) * 5).fill({ color: 0x6a5a4a, alpha: 0.18 + detRand(seed + i * 3) * 0.12 });
  }
}
...
function drawFrostBeam(
  ...
  for (let i = 1; i <= segments; i++) {
    const t = i / segments;
    const swirl = Math.sin(phase * 2.5 + t * 8 + seed) * (8 + t * 10);
  ...
  for (let i = 1; i <= segments; i++) {
    const t = i / segments;
    const wobble = Math.sin(phase * 4 + i * 1.1 + seed) * 3;
    stream.lineTo(sx + dx * t + nx * wobble * 0.08, sy + dy * t + ny * wobble * 0.08);
  }
  stream.stroke({ width: Math.max(2, startW * 0.45), color: 0xe8fcff, alpha: 0.9 });

  const outer = strokePool.acquire();
  outer.moveTo(sx, sy);
  for (let i = 1; i <= segments; i++) {
    const t = i / segments;
    const wobble = Math.sin(phase * 3.2 + i * 0.9 + seed + 1) * 5;
  ...
  for (let i = 0; i < 10; i++) {
    ...
    const drift = Math.sin(phase * 1.8 + i) * 10;
    const px = sx + dx * t + nx * drift;
    const py = sy + dy * t + ny * drift + Math.sin(phase * 2 + i) * 4;
    fillPool.acquire().circle(px, py, 1 + detRand(seed + i) * 1.5).fill({ color: 0xffffff, alpha: 0.5 + detRand(seed + i * 4) * 0.4 });
  }
  ...
  for (let i = 0; i < 6; i++) {
    ...
    fillPool.acquire().circle(ex + Math.cos(ang) * dist, ey + Math.sin(ang) * dist, 2 + detRand(seed + i) * 4).fill({ color: 0xe8fcff, alpha: 0.15 });
  }
  ...
  for (let i = 0; i < 6; i++) {
    ...
    if (Math.sin(phase * 10 + i * 3 + seed) > 0.2) {
      fillPool.acquire().circle(px + nx * detRand(seed + i) * 6, py + ny * detRand(seed + i + 1) * 6, 1.2).fill({ color: 0xffffff, alpha: 0.9 });
    }
  }
}
...
export function renderTowerBeams(
  ...
  for (const e of state.entities.values()) {
    ...
    if (beam.kind === 'flame') {
      drawFlameBeam(strokePool, fillPool, src.x, src.y, dst.x, dst.y, beam.startWidth, beam.endWidth, phase, seed);
    } else {
      drawFrostBeam(strokePool, fillPool, src.x, src.y, dst.x, dst.y, beam.startWidth, beam.endWidth, phase, seed);
    }
  }
}
