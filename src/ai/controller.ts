...
import { len, distSq } from '../sim/math';
...
function findEnemySanctum(state: GameState, owner: PlayerId): Entity | null {
  for (const e of state.entities.values()) {
    if (e.kind === 'building' && e.defId === 'sanctum' && isAlive(e) && isEnemy(state, owner, e.owner)) return e;
  }
  return null;
}
...
function idleCombat(combat: Entity[]): EntityId[] {
...
function decideForPlayer(state: GameState, services: SimServices, p: Player, diff: AiParams, cmds: Command[]): void {
  ...
  const combat = own.filter((e) => e.kind === 'unit' && e.carryMax === undefined && isAlive(e));
  ...
  if (
    ...
    const spot = findPlacement(services, sanctum.pos.x, sanctum.pos.y, turretDef.footprint);
    if (spot) {
...
function produceArmy(state: GameState, services: SimServices, p: Player, diff: AiParams, cmds: Command[]): void {
  ...
  const combat = ownedBy(state, p.id).filter((e) => e.kind === 'unit' && e.carryMax === undefined);

  const tryProduce = (building: Entity | undefined, defId: string): boolean => {
...
function pickAttackTarget(state: GameState, owner: PlayerId, from: { x: number; y: number }): Entity | null {
